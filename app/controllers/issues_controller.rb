...
class IssuesController < ApplicationController
  ...
  def update
    return unless update_issue_from_params
    if @issue.tracker_id==5 && @issue.parent_id != nil
      ...
      if @issues.count == 1
        unless @issue.valor_peso >= 100 || @issue.valor_peso <= 0
    ...
    if @issue.tracker_id==5 && @issue.parent_id != nil
      ...
      if @issue.tracker_id == 5
        if total == 100
          ...
          if @issue.valor_peso > 100 || @issue.valor_peso < 0
            flash.now[:notice] = l(:notice_value_out_of_range)
            render "update_valor_peso"
          else
            if total == 100
              respond_to do |format|
